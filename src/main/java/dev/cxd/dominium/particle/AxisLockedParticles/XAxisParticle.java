...
public class XAxisParticle extends SpriteBillboardParticle {
    private int delay;

    XAxisParticle(ClientWorld world, double x, double y, double z, int delay, SpriteProvider spriteProvider) {
        super(world, x, y, z);
        this.scale = 0.0F;
        this.delay = delay;
    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float tickDelta) {
        ...
        this.buildGeometry(vertexConsumer, camera, tickDelta, (quaternion) -> {
            quaternion.mul(new Quaternionf().rotationY((float) (Math.PI / 2)));
        });

        this.buildGeometry(vertexConsumer, camera, tickDelta, (quaternion) -> {
            quaternion.mul(new Quaternionf().rotationY((float) (Math.PI / 2 + Math.PI)));
        });
    }
    ...
    public static class Factory implements ParticleFactory<DefaultParticleType> {
        ...
        public Particle createParticle(DefaultParticleType particleType, ClientWorld clientWorld,
            ...
            XAxisParticle particle = new XAxisParticle(clientWorld, d, e, f, 0, this.spriteProvider);
